...
#define UIPC_MSG_ERROR 0xFF
    ...
    typedef struct UIPCShm UIPCShm;

    /** Create a new shared memory region of `size` bytes. NULL on failure. */
    UIPCShm *uipc_shm_create(const char *name, uint64_t size);

    /** Open an existing shared memory region. Caller must know the size. */
    UIPCShm *uipc_shm_open(const char *name, uint64_t size);

    /** Get a raw pointer to the shared memory region. */
    uint8_t *uipc_shm_ptr(UIPCShm *shm);

    /** Get the size of the region in bytes. */
    uint64_t uipc_shm_size(UIPCShm *shm);

    /** Unmap and close (does not unlink the segment). */
    void uipc_shm_close(UIPCShm *shm);

    /** Unlink the shared memory segment from the filesystem. */
    void uipc_shm_unlink(UIPCShm *shm);
