...
export function encode(msg: FlatMsg): Uint8Array {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    view.setUint16(pos, kb.length, true);
    pos += 2;
    out.set(kb, pos);
    pos += kb.length;
    view.setUint32(pos, vb.length, true);
    pos += 4;
    out.set(vb, pos);
    pos += vb.length;
  }
...
export function decode(buf: Uint8Array): FlatMsg {
  ...
  for (let i = 0; i < count; i++) {
    ...
    off += 2;
    const key = DEC.decode(buf.subarray(off, off + keyLen));
    off += keyLen;
    const valLen = view.getUint32(off, true);
    off += 4;
    const valBytes = buf.subarray(off, off + valLen);
    off += valLen;
...
function decodeValue(tag: number, b: Uint8Array): FlatValue {
  ...
  switch (tag) {
    ...
    case TAG.i64: {
      const lo = BigInt(view.getUint32(0, true));
      const hi = BigInt(view.getInt32(4, true));
