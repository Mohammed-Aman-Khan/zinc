...
class RingPool {
  ...
  addChannel(config) {
    if (this.#channels.has(config.name)) throw new Error(`Duplicate: ${config.name}`);
    ...
    this.#channels.set(config.name, {
      config, reqRing, repRing,
      stats: { sent: 0n, received: 0n, errors: 0n,
               latencies: new Array(this.#latBufSize).fill(0),
               latIdx: 0, lastPing: 0, healthy: true },
    });
    return this;
  }
  ...
  nextWorker() {
    ...
    for (let i = 0; i < this.#workers.length; i++) {
      ...
      if (entry?.stats.healthy) return { name, req: entry.reqRing, rep: entry.repRing };
    }
    throw new Error("No healthy workers");
  }

  recordSend(name)              { const e=this.#channels.get(name); if(e) e.stats.sent++; }
  recordRecv(name, latUs)       { const e=this.#channels.get(name); if(!e)return; e.stats.received++; e.stats.latencies[e.stats.latIdx]=latUs; e.stats.latIdx=(e.stats.latIdx+1)%this.#latBufSize; }
  recordError(name)             { const e=this.#channels.get(name); if(e){e.stats.errors++;e.stats.healthy=false;} }

  markHealthy(name, healthy)    { const e=this.#channels.get(name); if(e) e.stats.healthy=healthy; }
  ...
  allStats() {
    return Array.from(this.#channels.entries()).map(([name, e]) => {
      ...
      return { name, sent: e.stats.sent, received: e.stats.received,
               errors: e.stats.errors, avgLatencyUs: avg, lastPingUs: e.stats.lastPing, healthy: e.stats.healthy };
    });
  }

  close() { for(const n of [...this.#channels.keys()]) this.removeChannel(n); }
...
function throws(fn, substr) {
  ...
  catch(e) { if (!e.message.includes(substr)) throw new Error(`wrong error: ${e.message}`); }
}
...
test("addChannel creates req+rep rings", () => {
  const created = [];
  const pool = new RingPool((name, create) => { created.push({name, create}); return makeRing(name); });
  pool.addChannel({ name: "ch1", reqRingName: "/req1", repRingName: "/rep1", create: true });
  if (created.length !== 2) throw new Error(`Expected 2 rings, got ${created.length}`);
  if (created[0].name !== "/req1" || !created[0].create)  throw new Error("req ring wrong");
  if (created[1].name !== "/rep1" ||  created[1].create)  throw new Error("rep ring wrong (should be client)");
  pool.close();
});

test("channel() returns req/rep pair", () => {
  const pool = new RingPool((name) => makeRing(name));
  pool.addChannel({ name: "ch", reqRingName: "/r", repRingName: "/p", create: false });
...
test("duplicate channel name throws", () => {
  ...
  pool.addChannel({ name: "ch", reqRingName: "/r", repRingName: "/p", create: false });
  throws(() => pool.addChannel({ name: "ch", reqRingName: "/r2", repRingName: "/p2", create: false }), "Duplicate");
  pool.close();
});
...
test("addWorker + nextWorker round-robins", () => {
  const pool = new RingPool(() => makeRing());
  pool.addChannel({ name: "w1", reqRingName: "/r1", repRingName: "/p1", create: false });
  pool.addChannel({ name: "w2", reqRingName: "/r2", repRingName: "/p2", create: false });
  pool.addChannel({ name: "w3", reqRingName: "/r3", repRingName: "/p3", create: false });
...
test("recordSend increments counter", () => {
  const pool = new RingPool(() => makeRing());
  pool.addChannel({ name: "ch", reqRingName: "/r", repRingName: "/p", create: false });
  pool.recordSend("ch"); pool.recordSend("ch"); pool.recordSend("ch");
...
test("recordRecv computes average latency", () => {
  const pool = new RingPool(() => makeRing());
  pool.addChannel({ name: "ch", reqRingName: "/r", repRingName: "/p", create: false });
  pool.recordRecv("ch", 100); pool.recordRecv("ch", 200); pool.recordRecv("ch", 300);
  const stats = pool.allStats().find(s => s.name === "ch");
...
test("removeChannel decrements count", () => {
  ...
  if (pool.channelCount !== 0) throw new Error(`count after remove=${pool.channelCount}`);
});
...
test("removeChannel also removes from workers", () => {
  ...
  if (pool.workerCount !== 1) throw new Error(`post-remove count=${pool.workerCount}`);
  // nextWorker should now only return w2
  if (pool.nextWorker().name !== "w2") throw new Error("wrong worker after remove");
  pool.close();
});

test("close() empties all channels", () => {
  const pool = new RingPool(() => makeRing());
  pool.addChannel({ name: "a", reqRingName: "/ra", repRingName: "/pa", create: false });
  pool.addChannel({ name: "b", reqRingName: "/rb", repRingName: "/pb", create: false });
  pool.close();
...
test("can re-add channel after remove", () => {
  ...
  pool.addChannel({ name: "ch", reqRingName: "/r", repRingName: "/p", create: false });
  pool.removeChannel("ch");
  pool.addChannel({ name: "ch", reqRingName: "/r2", repRingName: "/p2", create: false });
...
test("recovering unhealthy channel back to healthy", () => {
  const pool = new RingPool(() => makeRing());
  pool.addChannel({ name: "w", reqRingName: "/r", repRingName: "/p", create: false });
  pool.addWorker("w");
  pool.recordError("w");
  // simulate recovery
...
for (let i = 0; i < 16; i++) {
  bigPool.addChannel({ name: `w${i}`, reqRingName: `/r${i}`, repRingName: `/p${i}`, create: false });
...
console.log(`   in ${elapsed.toFixed(1)}ms → ${(N / (elapsed / 1000) / 1e6).toFixed(2)}M/sec`);
