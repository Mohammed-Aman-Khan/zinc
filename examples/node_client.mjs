...
class NodeRPCClient {
  ...
  // Tiny encode/decode matching protocol/flat_msg.ts format.
    ...
    for (const [key, val] of fields) {
      ...
      f.writeUInt16LE(keyBytes.length, off);
      off += 2;
      f.set(keyBytes, off);
      off += keyBytes.length;
      f.writeUInt32LE(valPart.length, off);
      off += 4;
      f.set(valPart, off);
      parts.push(f);
    }
  ...
  #tagFor(v) {
    ...
    if (typeof v === "number") {
      ...
      b.writeDoubleLELE?.(v, 0) ?? b.writeDoubleLE(v, 0);
      return b;
    }
    if (typeof v === "bigint") {
      const b = Buffer.alloc(8);
  ...
  #decode(buf) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      off += 4;
      const valBuf = buf.subarray(off, off + valLen);
      off += valLen;
  ...
  #decodeVal(tag, b) {
    ...
    switch (tag) {
      ...
      case 0x09:
        return new DataView(b.buffer).getBigInt64(b.byteOffset, true);
      case 0x05:
